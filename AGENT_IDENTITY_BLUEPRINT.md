# Aegis — A Build Blueprint for Delegated Agent Identity
...
>
> "Aegis" is a working codename for the project. The agent's identity object is called a
...
Aegis is built on the opposite primitive: **delegation**. The agent has its own identity —
...
```jsonc
{
  ...
  "displayName": "Aegis Agent — Research",
  "created": "2026-06-12T10:00:00Z",
  "publicKey": { "type": "Ed25519", "jwk": { ... } },
  "serviceEndpoints": {
    "auditLog": "https://aegis.local/audit/AGENT",
...
statement of what an agent may spend") so that Aegis is payment-ecosystem-compatible from day
one — an Aegis payment Mandate can wrap an AP2 mandate envelope and settle over Mastercard
...
- **Observability bonus:** the agent's inbox is fully owned and logged by Aegis, unlike the
...
- **The channel:** push notification to the phone (you have this), with deep-link into the
  trusted Aegis app showing full context: "Agent *Research* wants to pay **$84.00** to
...
Aegis makes an agent **as capable as a human assistant with a company card and a scoped login**
...
1. **Semantic intent verification (§9):** cryptography proves *who* and *what's authorized*, not
   ...
   standards survey calls this the #1 open gap. Aegis mitigates (tight Mandates, STEP_UP on
...
3. **Standards churn:** AP2, ACP, MPP, x402, Web Bot Auth, MCP-I, Cedar are all moving. Aegis is
...
4. **Continuous / probabilistic identity:** the field is moving from binary credentials toward
   ...
   reputation hooks (§9) are where Aegis grows into that.
